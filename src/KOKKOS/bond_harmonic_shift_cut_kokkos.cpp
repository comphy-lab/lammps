...
#include "bond_harmonic_shift_cut_kokkos.h"
...
BondHarmonicShiftCutKokkos<DeviceType>::BondHarmonicShiftCutKokkos(LAMMPS *lmp) : BondHarmonicShiftCut(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
BondHarmonicShiftCutKokkos<DeviceType>::~BondHarmonicShiftCutKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void BondHarmonicShiftCutKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondHarmonicShiftCutCompute<1,1> >(0,nbondlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondHarmonicShiftCutCompute<0,1> >(0,nbondlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondHarmonicShiftCutCompute<1,0> >(0,nbondlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondHarmonicShiftCutCompute<0,0> >(0,nbondlist),*this);
    }
  }
...
void BondHarmonicShiftCutKokkos<DeviceType>::operator()(TagBondHarmonicShiftCutCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  KK_FLOAT ebond = 0.0;
  if (eflag) {
    const KK_FLOAT dr2 = d_r0[type] - d_r1[type];
    ebond = d_k[type] * (dr * dr - dr2 * dr2);
  }
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondHarmonicShiftCutKokkos<DeviceType>::operator()(TagBondHarmonicShiftCutCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagBondHarmonicShiftCutCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void BondHarmonicShiftCutKokkos<DeviceType>::allocate()
{
  BondHarmonicShiftCut::allocate();

  int n = atom->nbondtypes;
  k_k = DAT::tdual_kkfloat_1d("BondHarmonicShiftCut::k",n+1);
  k_r0 = DAT::tdual_kkfloat_1d("BondHarmonicShiftCut::r0",n+1);
  k_r1 = DAT::tdual_kkfloat_1d("BondHarmonicShiftCut::r1",n+1);

  d_k = k_k.template view<DeviceType>();
  d_r0 = k_r0.template view<DeviceType>();
  d_r1 = k_r1.template view<DeviceType>();
}
...
template<class DeviceType>
void BondHarmonicShiftCutKokkos<DeviceType>::coeff(int narg, char **arg)
{
  BondHarmonicShiftCut::coeff(narg, arg);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    k_r1.view_host()[i] = static_cast<KK_FLOAT>(r1[i]);
  }
...
template<class DeviceType>
void BondHarmonicShiftCutKokkos<DeviceType>::read_restart(FILE *fp)
{
  BondHarmonicShiftCut::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondHarmonicShiftCutKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
