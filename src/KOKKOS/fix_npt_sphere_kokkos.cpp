...
#include "fix_npt_sphere_kokkos.h"
...
FixNPTSphereKokkos<DeviceType>::FixNPTSphereKokkos(LAMMPS *lmp, int narg, char **arg) :
    FixNHSphereKokkos<DeviceType>(lmp, narg, arg)
{
  if (!this->tstat_flag)
    this->error->all(FLERR, "Temperature control must be used with fix npt/sphere/kk");
  if (!this->pstat_flag)
    this->error->all(FLERR, "Pressure control must be used with fix npt/sphere/kk");
...
namespace LAMMPS_NS {
template class FixNPTSphereKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class FixNPTSphereKokkos<LMPHostType>;
#endif
}
