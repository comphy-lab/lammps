...
#include "pair_mm3_switch3_coulgauss_long_kokkos.h"
...
PairMM3Switch3CoulGaussLongKokkos<DeviceType>::PairMM3Switch3CoulGaussLongKokkos(LAMMPS *lmp)
  : PairMM3Switch3CoulGaussLong(lmp)
{
  respa_enable = 0;
...
template<class DeviceType>
PairMM3Switch3CoulGaussLongKokkos<DeviceType>::~PairMM3Switch3CoulGaussLongKokkos()
{
  if (copymode) return;
...
template<class DeviceType>
void PairMM3Switch3CoulGaussLongKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  ev = pair_compute<PairMM3Switch3CoulGaussLongKokkos<DeviceType>,void>
...
/* ----------------------------------------------------------------------
   MM3 exp-r^6 pair force with Switch3 truncation
------------------------------------------------------------------------- */
...
KK_FLOAT PairMM3Switch3CoulGaussLongKokkos<DeviceType>::
              ...
              const int& itype, const int& jtype) const
{
  ...
  const KK_FLOAT lj1_val = STACKPARAMS ? m_params[itype][jtype].lj1 : params(itype,jtype).lj1;
  ...
  // Exponential repulsion + r^-6 attraction: forcelj = expb*lj1*r - 6*lj4*r6inv
  const KK_FLOAT expb = lj3_val * Kokkos::exp(-lj1_val * r);
  KK_FLOAT forcelj = expb*lj1_val*r - static_cast<KK_FLOAT>(6.0)*lj4_val*r6inv;
  KK_FLOAT evdwl = expb - lj4_val*r6inv - offset_val;
...
/* ----------------------------------------------------------------------
   MM3 exp-r^6 pair potential energy with Switch3 truncation
------------------------------------------------------------------------- */
...
template<class DeviceType>
void PairMM3Switch3CoulGaussLongKokkos<DeviceType>::allocate()
{
  PairMM3Switch3CoulGaussLong::allocate();
  ...
  k_params = Kokkos::DualView<params_mm3_sw3_cg**,Kokkos::LayoutRight,DeviceType>(
    "PairMM3Switch3CoulGaussLong::params",n+1,n+1);
...
template<class DeviceType>
void PairMM3Switch3CoulGaussLongKokkos<DeviceType>::init_style()
{
  PairMM3Switch3CoulGaussLong::init_style();
...
template<class DeviceType>
double PairMM3Switch3CoulGaussLongKokkos<DeviceType>::init_one(int i, int j)
{
  double cutone = PairMM3Switch3CoulGaussLong::init_one(i,j);
