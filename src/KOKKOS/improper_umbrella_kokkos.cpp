...
#include "improper_umbrella_kokkos.h"
#include <cmath>
...
ImproperUmbrellaKokkos<DeviceType>::ImproperUmbrellaKokkos(LAMMPS *lmp) : ImproperUmbrella(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
ImproperUmbrellaKokkos<DeviceType>::~ImproperUmbrellaKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void ImproperUmbrellaKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagImproperUmbrellaCompute<1,1> >(0,nimproperlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagImproperUmbrellaCompute<0,1> >(0,nimproperlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagImproperUmbrellaCompute<1,0> >(0,nimproperlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagImproperUmbrellaCompute<0,0> >(0,nimproperlist),*this);
    }
  }
...
void ImproperUmbrellaKokkos<DeviceType>::operator()(TagImproperUmbrellaCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const
{
  ...
  KK_FLOAT domega, c, a, s, projhfg, dhax, dhay, dhaz, dahx, dahy, dahz, cotphi;
  ...
  arx = ax*rar; ary = ay*rar; arz = az*rar;
  hrx = vb3x*rhr; hry = vb3y*rhr; hrz = vb3z*rhr;
  ...
  if (d_w0[type] == static_cast<KK_FLOAT>(0.0)) {
  ...
  a = a*cotphi;
  dhax = hrx - c*arx; dhay = hry - c*ary; dhaz = hrz - c*arz;
  dahx = arx - c*hrx; dahy = ary - c*hry; dahz = arz - c*hrz;
  ...
  if (EVFLAG) {
    // recompute vb vectors for virial tally (standard 4-body convention)
    const KK_FLOAT vb1xv = x(i1,0) - x(i2,0);
    const KK_FLOAT vb1yv = x(i1,1) - x(i2,1);
    const KK_FLOAT vb1zv = x(i1,2) - x(i2,2);
    const KK_FLOAT vb2xv = x(i3,0) - x(i2,0);
    const KK_FLOAT vb2yv = x(i3,1) - x(i2,1);
    const KK_FLOAT vb2zv = x(i3,2) - x(i2,2);
    const KK_FLOAT vb3xv = x(i4,0) - x(i3,0);
    const KK_FLOAT vb3yv = x(i4,1) - x(i3,1);
    const KK_FLOAT vb3zv = x(i4,2) - x(i3,2);
    ev_tally(ev,i1,i2,i3,i4,eimproper,f1,f2,f4,
             vb1xv,vb1yv,vb1zv, vb2xv,vb2yv,vb2zv, vb3xv,vb3yv,vb3zv);
  }
}
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void ImproperUmbrellaKokkos<DeviceType>::operator()(TagImproperUmbrellaCompute<NEWTON_BOND,EVFLAG>, const int &n) const
{
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagImproperUmbrellaCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void ImproperUmbrellaKokkos<DeviceType>::allocate()
{
  ImproperUmbrella::allocate();
  int n = atom->nimpropertypes;
  k_kw = DAT::tdual_kkfloat_1d("ImproperUmbrella::kw",n+1);
  k_w0 = DAT::tdual_kkfloat_1d("ImproperUmbrella::w0",n+1);
  k_C  = DAT::tdual_kkfloat_1d("ImproperUmbrella::C",n+1);
  d_kw = k_kw.template view<DeviceType>();
  d_w0 = k_w0.template view<DeviceType>();
  d_C  = k_C.template view<DeviceType>();
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void ImproperUmbrellaKokkos<DeviceType>::coeff(int narg, char **arg)
{
  ImproperUmbrella::coeff(narg, arg);
  int ilo,ihi;
  utils::bounds(FLERR,arg[0],1,atom->nimpropertypes,ilo,ihi,error);
  for (int i = ilo; i <= ihi; i++) {
    k_kw.view_host()[i] = static_cast<KK_FLOAT>(kw[i]);
    k_w0.view_host()[i] = static_cast<KK_FLOAT>(w0[i]);
    k_C.view_host()[i]  = static_cast<KK_FLOAT>(C[i]);
  }
  k_kw.modify_host();
  k_w0.modify_host();
  k_C.modify_host();
...
template<class DeviceType>
void ImproperUmbrellaKokkos<DeviceType>::read_restart(FILE *fp)
{
  ImproperUmbrella::read_restart(fp);
  int n = atom->nimpropertypes;
  for (int i = 1; i <= n; i++) {
    k_kw.view_host()[i] = static_cast<KK_FLOAT>(kw[i]);
    k_w0.view_host()[i] = static_cast<KK_FLOAT>(w0[i]);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void ImproperUmbrellaKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i1, const int i2,
