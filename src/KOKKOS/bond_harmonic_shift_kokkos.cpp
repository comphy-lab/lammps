...
#include "bond_harmonic_shift_kokkos.h"
...
BondHarmonicShiftKokkos<DeviceType>::BondHarmonicShiftKokkos(LAMMPS *lmp) : BondHarmonicShift(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
BondHarmonicShiftKokkos<DeviceType>::~BondHarmonicShiftKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void BondHarmonicShiftKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondHarmonicShiftCompute<1,1> >(0,nbondlist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagBondHarmonicShiftCompute<0,1> >(0,nbondlist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondHarmonicShiftCompute<1,0> >(0,nbondlist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagBondHarmonicShiftCompute<0,0> >(0,nbondlist),*this);
    }
  }
...
void BondHarmonicShiftKokkos<DeviceType>::operator()(TagBondHarmonicShiftCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondHarmonicShiftKokkos<DeviceType>::operator()(TagBondHarmonicShiftCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagBondHarmonicShiftCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void BondHarmonicShiftKokkos<DeviceType>::allocate()
{
  BondHarmonicShift::allocate();

  int n = atom->nbondtypes;
  k_k = DAT::tdual_kkfloat_1d("BondHarmonicShift::k",n+1);
  k_r0 = DAT::tdual_kkfloat_1d("BondHarmonicShift::r0",n+1);
  k_r1 = DAT::tdual_kkfloat_1d("BondHarmonicShift::r1",n+1);
...
template<class DeviceType>
void BondHarmonicShiftKokkos<DeviceType>::coeff(int narg, char **arg)
{
  BondHarmonicShift::coeff(narg, arg);
...
template<class DeviceType>
void BondHarmonicShiftKokkos<DeviceType>::read_restart(FILE *fp)
{
  BondHarmonicShift::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void BondHarmonicShiftKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
