...
BondQuarticExpKokkos<DeviceType>::BondQuarticExpKokkos(LAMMPS *lmp) : BondQuarticExp(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
BondQuarticExpKokkos<DeviceType>::~BondQuarticExpKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void BondQuarticExpKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(
        Kokkos::RangePolicy<DeviceType, TagBondQuarticExpCompute<1,1> >(0,nbondlist),
        *this, ev);
    } else {
      Kokkos::parallel_reduce(
        Kokkos::RangePolicy<DeviceType, TagBondQuarticExpCompute<0,1> >(0,nbondlist),
        *this, ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(
        Kokkos::RangePolicy<DeviceType, TagBondQuarticExpCompute<1,0> >(0,nbondlist),
        *this);
    } else {
      Kokkos::parallel_for(
        Kokkos::RangePolicy<DeviceType, TagBondQuarticExpCompute<0,0> >(0,nbondlist),
        *this);
    }
  }
...
void BondQuarticExpKokkos<DeviceType>::operator()(
  TagBondQuarticExpCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const
{
  ...
  // Quartic force and energy

  KK_FLOAT ebond = 0.0;
  KK_FLOAT fbond = 0.0;

  const KK_FLOAT de_bond =
    static_cast<KK_FLOAT>(2.0)*d_k2[type]*dr +
...
KOKKOS_INLINE_FUNCTION
void BondQuarticExpKokkos<DeviceType>::operator()(
  TagBondQuarticExpCompute<NEWTON_BOND,EVFLAG>, const int &n) const
{
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(
    TagBondQuarticExpCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void BondQuarticExpKokkos<DeviceType>::allocate()
{
  BondQuarticExp::allocate();
}
...
template<class DeviceType>
void BondQuarticExpKokkos<DeviceType>::coeff(int narg, char **arg)
{
  BondQuarticExp::coeff(narg, arg);

  const int n = atom->nbondtypes;

  DAT::tdual_kkfloat_1d k_r0("BondQuarticExp::r0",n+1);
  DAT::tdual_kkfloat_1d k_k2("BondQuarticExp::k2",n+1);
  DAT::tdual_kkfloat_1d k_k3("BondQuarticExp::k3",n+1);
  DAT::tdual_kkfloat_1d k_k4("BondQuarticExp::k4",n+1);
  DAT::tdual_kkfloat_1d k_A ("BondQuarticExp::A", n+1);
  DAT::tdual_kkfloat_1d k_B ("BondQuarticExp::B", n+1);
  ...
  for (int i = 1; i <= n; i++) {
    ...
    k_B .view_host()[i] = static_cast<KK_FLOAT>(B[i]);
  }
...
template<class DeviceType>
void BondQuarticExpKokkos<DeviceType>::read_restart(FILE *fp)
{
  BondQuarticExp::read_restart(fp);
...
template<class DeviceType>
KOKKOS_INLINE_FUNCTION
void BondQuarticExpKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int &i, const int &j,
