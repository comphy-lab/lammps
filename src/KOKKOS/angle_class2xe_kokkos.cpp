...
#include "angle_class2xe_kokkos.h"
...
AngleClass2xeKokkos<DeviceType>::AngleClass2xeKokkos(LAMMPS *lmp) : AngleClass2xe(lmp)
{
  kokkosable = 1;
...
template<class DeviceType>
AngleClass2xeKokkos<DeviceType>::~AngleClass2xeKokkos()
{
  if (!copymode) {
...
template<class DeviceType>
void AngleClass2xeKokkos<DeviceType>::compute(int eflag_in, int vflag_in)
{
  eflag = eflag_in;
  vflag = vflag_in;
  ...
  k_bb_d0.template sync<DeviceType>();
  k_bb_alpha.template sync<DeviceType>();
  k_bb_r1.template sync<DeviceType>();
  k_bb_r2.template sync<DeviceType>();
  k_ba_d1.template sync<DeviceType>();
  k_ba_d2.template sync<DeviceType>();
  k_ba_alpha1.template sync<DeviceType>();
  k_ba_alpha2.template sync<DeviceType>();
  ...
  if (evflag) {
    if (newton_bond) {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleClass2xeCompute<1,1> >(0,nanglelist),*this,ev);
    } else {
      Kokkos::parallel_reduce(Kokkos::RangePolicy<DeviceType, TagAngleClass2xeCompute<0,1> >(0,nanglelist),*this,ev);
    }
  } else {
    if (newton_bond) {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleClass2xeCompute<1,0> >(0,nanglelist),*this);
    } else {
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType, TagAngleClass2xeCompute<0,0> >(0,nanglelist),*this);
    }
  }
...
void AngleClass2xeKokkos<DeviceType>::operator()(TagAngleClass2xeCompute<NEWTON_BOND,EVFLAG>, const int &n, EV_FLOAT& ev) const {
  ...
  const KK_FLOAT a_ang = -de_angle*s;
  const KK_FLOAT a11_ang = a_ang*c / rsq1;
  const KK_FLOAT a12_ang = -a_ang / (r1*r2);
  const KK_FLOAT a22_ang = a_ang*c / rsq2;

  KK_FLOAT f1[3],f3[3];
  f1[0] = a11_ang*delx1 + a12_ang*delx2;
  f1[1] = a11_ang*dely1 + a12_ang*dely2;
  f1[2] = a11_ang*delz1 + a12_ang*delz2;
  f3[0] = a22_ang*delx2 + a12_ang*delx1;
  f3[1] = a22_ang*dely2 + a12_ang*dely1;
  f3[2] = a22_ang*delz2 + a12_ang*delz1;

  KK_FLOAT eangle = static_cast<KK_FLOAT>(0.0);
  if (eflag)
    eangle = d_k2[type]*dtheta2 + d_k3[type]*dtheta3 + d_k4[type]*dtheta4;
  ...
  if (eflag)
    eangle += d_bb_d0[type]*(static_cast<KK_FLOAT>(1.0) - bb_ralpha1)*(static_cast<KK_FLOAT>(1.0) - bb_ralpha2);

  // force & energy for bond-angle term

  const KK_FLOAT dr1_ba = r1 - d_ba_r1[type];
  const KK_FLOAT dr2_ba = r2 - d_ba_r2[type];
  const KK_FLOAT ba_ralpha1 = exp(-d_ba_alpha1[type]*dr1_ba);
  const KK_FLOAT ba_ralpha2 = exp(-d_ba_alpha2[type]*dr2_ba);
  const KK_FLOAT aa1 = d_ba_d1[type]*(static_cast<KK_FLOAT>(1.0) - ba_ralpha1)*s;
  const KK_FLOAT aa2 = d_ba_d2[type]*(static_cast<KK_FLOAT>(1.0) - ba_ralpha2)*s;
  ...
  const KK_FLOAT b1 = d_ba_d1[type]*d_ba_alpha1[type]*ba_ralpha1*dtheta/r1;
  const KK_FLOAT b2 = d_ba_d2[type]*d_ba_alpha2[type]*ba_ralpha2*dtheta/r2;
  ...
  if (eflag)
    eangle += d_ba_d1[type]*(static_cast<KK_FLOAT>(1.0) - ba_ralpha1)*dtheta +
              d_ba_d2[type]*(static_cast<KK_FLOAT>(1.0) - ba_ralpha2)*dtheta;
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleClass2xeKokkos<DeviceType>::operator()(TagAngleClass2xeCompute<NEWTON_BOND,EVFLAG>, const int &n) const {
  EV_FLOAT ev;
  this->template operator()<NEWTON_BOND,EVFLAG>(TagAngleClass2xeCompute<NEWTON_BOND,EVFLAG>(), n, ev);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
void AngleClass2xeKokkos<DeviceType>::allocate()
{
  AngleClass2xe::allocate();

  int n = atom->nangletypes;

  k_theta0   = DAT::tdual_kkfloat_1d("AngleClass2xe::theta0",n+1);
  k_k2       = DAT::tdual_kkfloat_1d("AngleClass2xe::k2",n+1);
  k_k3       = DAT::tdual_kkfloat_1d("AngleClass2xe::k3",n+1);
  k_k4       = DAT::tdual_kkfloat_1d("AngleClass2xe::k4",n+1);
  k_bb_d0    = DAT::tdual_kkfloat_1d("AngleClass2xe::bb_d0",n+1);
  k_bb_alpha = DAT::tdual_kkfloat_1d("AngleClass2xe::bb_alpha",n+1);
  k_bb_r1    = DAT::tdual_kkfloat_1d("AngleClass2xe::bb_r1",n+1);
  k_bb_r2    = DAT::tdual_kkfloat_1d("AngleClass2xe::bb_r2",n+1);
  k_ba_d1    = DAT::tdual_kkfloat_1d("AngleClass2xe::ba_d1",n+1);
  k_ba_d2    = DAT::tdual_kkfloat_1d("AngleClass2xe::ba_d2",n+1);
  k_ba_alpha1 = DAT::tdual_kkfloat_1d("AngleClass2xe::ba_alpha1",n+1);
  k_ba_alpha2 = DAT::tdual_kkfloat_1d("AngleClass2xe::ba_alpha2",n+1);
  k_ba_r1    = DAT::tdual_kkfloat_1d("AngleClass2xe::ba_r1",n+1);
  k_ba_r2    = DAT::tdual_kkfloat_1d("AngleClass2xe::ba_r2",n+1);
  ...
  d_bb_d0    = k_bb_d0.template view<DeviceType>();
  d_bb_alpha = k_bb_alpha.template view<DeviceType>();
  d_bb_r1    = k_bb_r1.template view<DeviceType>();
  d_bb_r2    = k_bb_r2.template view<DeviceType>();
  d_ba_d1    = k_ba_d1.template view<DeviceType>();
  d_ba_d2    = k_ba_d2.template view<DeviceType>();
  d_ba_alpha1 = k_ba_alpha1.template view<DeviceType>();
  d_ba_alpha2 = k_ba_alpha2.template view<DeviceType>();
...
template<class DeviceType>
void AngleClass2xeKokkos<DeviceType>::coeff(int narg, char **arg)
{
  AngleClass2xe::coeff(narg, arg);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    k_bb_d0.view_host()[i]    = static_cast<KK_FLOAT>(bb_d0[i]);
    k_bb_alpha.view_host()[i] = static_cast<KK_FLOAT>(bb_alpha[i]);
    k_bb_r1.view_host()[i]    = static_cast<KK_FLOAT>(bb_r1[i]);
    k_bb_r2.view_host()[i]    = static_cast<KK_FLOAT>(bb_r2[i]);
    k_ba_d1.view_host()[i]    = static_cast<KK_FLOAT>(ba_d1[i]);
    k_ba_d2.view_host()[i]    = static_cast<KK_FLOAT>(ba_d2[i]);
    k_ba_alpha1.view_host()[i] = static_cast<KK_FLOAT>(ba_alpha1[i]);
    k_ba_alpha2.view_host()[i] = static_cast<KK_FLOAT>(ba_alpha2[i]);
  ...
  k_k4.modify_host();
  k_bb_d0.modify_host();
  k_bb_alpha.modify_host();
  k_bb_r1.modify_host();
  k_bb_r2.modify_host();
  k_ba_d1.modify_host();
  k_ba_d2.modify_host();
  k_ba_alpha1.modify_host();
  k_ba_alpha2.modify_host();
  k_ba_r1.modify_host();
  k_ba_r2.modify_host();
}
...
template<class DeviceType>
void AngleClass2xeKokkos<DeviceType>::read_restart(FILE *fp)
{
  AngleClass2xe::read_restart(fp);
...
// NOLINTNEXTLINE
KOKKOS_INLINE_FUNCTION
void AngleClass2xeKokkos<DeviceType>::ev_tally(EV_FLOAT &ev, const int i, const int j, const int k,
...
namespace LAMMPS_NS {
template class AngleClass2xeKokkos<LMPDeviceType>;
#ifdef LMP_KOKKOS_GPU
template class AngleClass2xeKokkos<LMPHostType>;
#endif
}
