...
void fourier_poly(double tol, double tol_coeff, double &c, double &lambda, std::vector<double> &coeffs)
{
  prolc180(tol, c);
  ...
  for (int idof = 0; idof < dof; idof++) {
      for (int i = 0; i < order; i++) {
          fn_v[idof * order + i] = f(lambda, c0, c, nodes[i]);
      }
  }
