...
BondFENENMOMP::BondFENENMOMP(class LAMMPS *lmp)
  : BondFENENM(lmp), ThrOMP(lmp,THR_BOND)
{
...
void BondFENENMOMP::compute(int eflag, int vflag)
{
  ev_init(eflag,vflag);
...
void BondFENENMOMP::eval(int nfrom, int nto, ThrData * const thr)
{
  int i1,i2,n,type;
  double delx,dely,delz,ebond,fbond;
  double rsq,r0sq,rlogarg,r;
  ...
  for (n = nfrom; n < nto; n++) {
    ...
    // change cutoff from .1 to .02 so only bond lengths > 1.485 give the warning
