...
void BondQuarticExp::compute(int eflag, int vflag)
{
  int i1,i2,n,type;
...
double BondQuarticExp::equilibrium_distance(int i)
{
  return r0[i];
}
...
void BondQuarticExp::write_restart(FILE *fp)
{
...
void BondQuarticExp::read_restart(FILE *fp)
{
  allocate();
...
void BondQuarticExp::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nbondtypes; i++)
    fprintf(fp,"%d %g %g %g %g %g %g\n",i,r0[i],k2[i],k3[i],k4[i],A[i],B[i]);
}

/* ---------------------------------------------------------------------- */

double BondQuarticExp::single(int type, double rsq, int /*i*/, int /*j*/,
                            double &fforce)
{
  double r = sqrt(rsq);
  double dr = r - r0[type];
