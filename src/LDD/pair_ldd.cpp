...
// This can be moved if there's a better place for it elsewhere.
  ...
  {
    error->all(FLERR,"atomstyle ldd must be used with pair style ldd");
  }

  if (atom->ldd_ntypes != atom->ntypes)
  {
     error->all(FLERR,"ldd_ntypes doesn't match ntypes");
  }
...
#define LddIndicatorStyle(key,Class) \
  (*indicator_map)[#key] = &indicator_creator<Class>;
...
#define LddPotentialStyle(key,Class) \
  (*potential_map)[#key] = &potential_creator<Class>;
...
// Accordingly, we have to call ev_tally_xyz to properly update the virial tensor.
              if (evflag) ev_tally_xyz(i,j,nlocal,newton_pair,0.0,0.0,fxtmp,fytmp,fztmp,
                                                                        delx,dely,delz);
            }
...
template <typename T>
LddIndicator *PairLdd::indicator_creator(LAMMPS *lmp)
{
  return new T(lmp);
}
...
template <typename T>
LddPotential *PairLdd::potential_creator(LAMMPS *lmp)
{
  return new T(lmp);
}
/* ---------------------------------------------------------------------- */
...
/* make sure the proper number of arguments for this keyword are present */
      ...
      if (strcmp(arg[iarg+1],"yes") == 0) bSelf = true;
                       ...
                       double factor_lj, double &fforce)
{
  ...
  if (! ignore_pair[itype][jtype])
  {
    if ((Inds[itype][jtype]->r0 <= r_pair) &&
        (Inds[itype][jtype]->rc >= r_pair))
    {
  ...
  if (! ignore_pair[jtype][itype])
  {
    if ((Inds[jtype][itype]->r0 <= r_pair) &&
        (Inds[jtype][itype]->rc >= r_pair))
    {
      fforce += Inds[jtype][itype]->wp(r_pair) *
                Potls[jtype][itype]->f(local_dens[j][itype]);
...
void PairLdd::LDD_calculate_LDs() //
{
  ...
  for (ii = 0; ii < inum; ii++)
  {
    ...
    if ( ! ignore_me[itype] )
    {
      ...
      const int jnum = numneigh[i];
// self interaction
      if (self_interaction[itype][itype])
      {
...
void PairLdd::read_file(char * filename, int nelements)
{
  ...
  if (comm->me == 0)
  {
        ...
        if (lddinp_fp == nullptr) { error->one(FLERR, "Cannot open ldd input file {}: {}", filename, utils::getsyserror());}
  }
